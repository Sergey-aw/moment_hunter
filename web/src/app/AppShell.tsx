import { Trophy, UserRound, UsersRound, WavesLadder } from 'lucide-react'
  ...
  return (
    <main className="min-h-screen">
      <div className="mx-auto flex w-full max-w-7xl flex-col px-4 pb-24 pt-4 sm:px-6 lg:px-8 lg:pb-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <img
                ...
                className="mt-2 h-6 w-auto object-contain [filter:brightness(0)_invert(1)]"
              />
              <p className="mt-2 text-xs uppercase text-emerald-300">Live Prediction Arena</p>
            </div>
        ...
        <nav className="mb-6 mt-4 hidden gap-2 lg:flex">
          {navItems.map((item) => {
        ...
        <footer className="mt-8 rounded-xl border border-slate-700/70 bg-slate-900/60 px-4 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="inline-flex items-center gap-2">
              <img src="/logo.svg" alt="Moment Hunter logo" className="h-4 w-auto object-contain [filter:brightness(0)_invert(1)]" />
              <span className="text-xs text-slate-300">Moment Hunter</span>
            </div>

            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-300">
              {navItems.map((item) => (
                <Link key={`footer-${item.to}`} to={item.to} className="hover:text-emerald-200">
                  {item.label}
                </Link>
              ))}
              <Link to="/license" className="hover:text-emerald-200">
                License Agreement
              </Link>
            </div>
          </div>
